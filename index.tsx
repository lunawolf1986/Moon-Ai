...
const CharacterCard = ({ character, onStartChat }: { character: any, onStartChat: (id: string) => void, key?: React.Key }) => (
  <div className="bg-surface rounded-3xl p-4 shadow-xl border border-white/5 flex flex-col gap-3 hover:border-primary/40 transition-all group active:scale-[0.98]">
...
const App = () => {
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      ...
      case "create": return <CreateView initialMode={createViewMode} onCreateCharacter={(c: any) => { setCharacters([c, ...characters]); setActiveTab("library"); }} onCreatePersona={(p: any) => { setPersonas([p, ...personas]); setActiveTab("profile"); }} onBack={() => setActiveTab('for_you')} />;
      case "library": return <LibraryView chats={chats} characters={characters} personas={personas} />;
      case "profile": return <ProfileView personas={personas} activePersonaId={activePersonaId} setActivePersonaId={(id: string) => { setActivePersonaId(id); const p = personas.find(pers => pers.id === id); if (p) setAppToast(`Manifested: ${p.name}`); }} chats={chats} updatePersona={(p: any) => setPersonas(prev => prev.map(o => o.id === p.id ? p : o))} />;
  ...
  return (
    <div className="flex flex-col h-screen bg-darker text-white font-sans overflow-hidden">
      {!activeChatId && (
        <div className="h-16 px-6 border-b border-white/5 flex items-center justify-between bg-darker/80 backdrop-blur-xl z-50">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-xl font-black tracking-tighter uppercase">Moonai</h1>
          </div>
      ...
      {!activeChatId && (
        <div className="h-20 bg-surface/90 backdrop-blur-md border-t border-white/5 flex-none z-50">
          <div className="flex items-center h-full overflow-x-auto no-scrollbar px-2 space-x-1">
...
const NavItem = ({ id, label, icon: Icon, active, set }: any) => (
  <button onClick={() => set(id)} className={`flex flex-col items-center justify-center min-w-[65px] h-16 rounded-xl transition-all duration-200 ${active === id ? 'text-primary' : 'text-slate-500 hover:text-slate-300'}`}>
...
const ForYouView = ({ characters, onStartChat }: any) => (
  <div className="p-4 pb-24 space-y-10">
    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">{characters.slice(2).map((c: any) => <CharacterCard key={c.id} character={c} onStartChat={onStartChat} />)}</div>
    </section>
  </div>
);
...
const FeaturedView = ({ characters, onStartChat }: any) => (
  <div className="p-4 pb-24">
    <h1 className="text-3xl font-black mb-8 uppercase tracking-tighter">Originals</h1>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{characters.map((c: any) => <CharacterCard key={c.id} character={c} onStartChat={onStartChat} />)}</div>
  </div>
);
  ...
  return (
    <div className="p-4 pb-24">
      <div className="relative mb-8">
        ...
        <input type="text" placeholder="Search identities..." className="w-full bg-surface border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-white font-bold focus:outline-none focus:border-primary/50 transition-all placeholder:text-slate-600 shadow-inner" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-10">
        {availableTraits.map(tag => (
          <button key={tag} onClick={() => { vibrate(); setSelectedTraits(p => p.includes(tag) ? p.filter(t => t !== tag) : [...p, tag]); }} className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all border uppercase tracking-widest ${selectedTraits.includes(tag) ? "bg-primary border-primary text-white shadow-lg shadow-primary/20" : "bg-surface/30 border-white/5 text-slate-500"}`}>{tag}</button>
        ))}
      </div>
      <div className="grid grid-cols-1 gap-6">{filtered.length > 0 ? filtered.map((c: any) => <CharacterCard key={c.id} character={c} onStartChat={onStartChat} />) : <div className="flex flex-col items-center justify-center py-20 opacity-20"><Search size={48} /><p className="font-bold">No results found.</p></div>}</div>
    </div>
  );
};
...
const ChatListView = ({ chats, characters, onOpenChat }: any) => {
  ...
  return (
    <div className="p-4 pb-24">
      <h1 className="text-3xl font-black mb-8 uppercase tracking-tighter">Dialogues</h1>
      <div className="space-y-3">
        {chats.map((chat: ChatSession) => {
          ...
          return (
            <button key={chat.id} onClick={() => onOpenChat(chat.id)} className="w-full bg-surface/40 p-4 rounded-3xl flex items-center gap-4 hover:bg-surface transition-all border border-transparent hover:border-white/5 text-left active:scale-[0.98]">
...
const CharacterEditor = ({ initialData, onSave, onCancel, mode }: { initialData: Partial<Character>, onSave: (data: Partial<Character>) => void, onCancel: () => void, mode: "create" | "edit" }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-darker/95 backdrop-blur-3xl flex flex-col animate-in slide-in-from-right duration-300 overflow-hidden">
      {/* Header */}
      <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 bg-darker/50 sticky top-0 z-30">
        <button onClick={onCancel} className="text-slate-400 font-bold text-sm uppercase tracking-widest hover:text-white transition-colors">Cancel</button>
        <h1 className="text-base font-black uppercase tracking-widest text-white/80">{mode === 'create' ? 'Architect Identity' : 'Character Settings'}</h1>
        <button 
          onClick={() => { 
            vibrate();
            if (!name.trim()) { alert("Name is required"); return; }
            onSave({ name, tagline, subtitle, description, systemInstruction, memory, visibility, maturityLevel: maturity }); 
          }} 
          className="text-primary font-black text-sm uppercase tracking-widest hover:text-blue-400 transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-32">
        ...
        <div className="p-6 bg-primary/5 border-b border-white/5">
          <label className="text-[9px] font-black text-primary uppercase tracking-[0.3em] block mb-4">Real-time Preview</label>
          <div className="max-w-md mx-auto w-full">
            <CharacterCard character={{
              id: 'preview',
              name: name || 'New Entity',
              tagline: tagline || 'A new identity awaiting definition...',
        ...
        <div className="p-6 max-w-2xl mx-auto w-full space-y-12">
          {/* Profile Section */}
          <section className="space-y-6">
            <div className="flex items-center gap-6">
              <AbstractAvatar name={name || 'N'} colorClass={initialData.color || 'bg-indigo-600'} size="lg" initial={(name?.[0] || 'N').toUpperCase()} />
              <div className="flex-1 space-y-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Character Name</label>
                <input 
                  value={name} 
                  onChange={e => setName(e.target.value)} 
                  className="w-full bg-surface/30 border border-white/10 rounded-xl p-3 text-white font-bold text-lg focus:border-primary/50 outline-none transition-all shadow-inner"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">One-Liner Tagline</label>
              <input 
                value={tagline} 
                onChange={e => setTagline(e.target.value)} 
                className="w-full bg-surface/30 border border-white/10 rounded-xl p-3 text-white font-medium focus:border-primary/50 outline-none transition-all shadow-inner"
          ...
          <section className="space-y-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-primary/80 border-b border-primary/20 pb-2">Public Profile</h3>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Profile Description</label>
              <textarea 
                value={description} 
                onChange={e => { setDescription(e.target.value); handleAutoHeight(e as any); }} 
                onFocus={handleAutoHeight}
                className="w-full bg-surface/30 border border-white/10 rounded-xl p-4 text-white font-medium text-sm leading-relaxed focus:border-primary/50 outline-none transition-all resize-none min-h-[100px] shadow-inner"
                placeholder="Describe the character to others..."
              />
            </div>
          </section>
          ...
          {/* Attributes Section */}
          <section className="space-y-6">
            <h3 className="text-sm font-black uppercase tracking-widest text-primary/80 border-b border-primary/20 pb-2">Attributes</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Subtitle (Title)</label>
                <input 
                  value={subtitle} 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Visibility</label>
                <div className="flex bg-surface/30 p-1 rounded-xl border border-white/10">
                  <button onClick={() => setVisibility('public')} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${visibility === 'public' ? 'bg-primary text-white shadow-lg' : 'text-slate-500'}`}>Public</button>
                  <button onClick={() => setVisibility('private')} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${visibility === 'private' ? 'bg-primary text-white shadow-lg' : 'text-slate-500'}`}>Private</button>
                </div>
              </div>
            </div>

            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Maturity Spectrum</label>
              <div className="flex bg-surface/30 p-1.5 rounded-2xl border border-white/10 gap-1.5 shadow-inner">
                {[
                  ...
                  <button 
                    ...
                    className={`flex-1 flex flex-col items-center justify-center py-3 rounded-xl transition-all ${maturity === lvl.id ? `bg-primary text-white shadow-xl scale-105` : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}
                  >
                    <span className="text-sm font-black tracking-tight">{lvl.label}</span>
                    <span className="text-[8px] font-bold uppercase opacity-60">{lvl.desc}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <section className="space-y-6">
            <div className="flex items-center justify-between border-b border-primary/20 pb-2">
              <h3 className="text-sm font-black uppercase tracking-widest text-primary/80">Advanced Definitions</h3>
              <button 
                ...
                className="text-[10px] font-black text-primary uppercase flex items-center gap-2 hover:text-white transition-colors"
              >
                {isAILoading ? <RefreshCw className="animate-spin" size={12} /> : <Wand2 size={12} />} AI Gen
              </button>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Behavioral Logic</label>
              <textarea 
                value={systemInstruction} 
                onChange={e => { setSystemInstruction(e.target.value); handleAutoHeight(e as any); }} 
                onFocus={handleAutoHeight}
                className="w-full bg-darker border border-white/5 rounded-2xl p-6 text-white font-mono text-xs leading-relaxed focus:border-primary/50 outline-none transition-all resize-none min-h-[300px] shadow-inner"
                placeholder="How they speak, secrets, and world logic..."
              />
            </div>
          </section>
          ...
          {/* Memory Log */}
          <section className="space-y-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-primary/80 border-b border-primary/20 pb-2">Persistence</h3>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Legacy Node</label>
              <textarea 
                value={memory} 
                onChange={e => { setMemory(e.target.value); handleAutoHeight(e as any); }} 
                onFocus={handleAutoHeight}
                className="w-full bg-surface/20 border border-white/5 rounded-xl p-4 text-white font-medium text-xs leading-relaxed focus:border-primary/50 outline-none transition-all resize-none min-h-[120px] shadow-inner"
                placeholder="Persistent narrative facts..."
              />
            </div>
          </section>
        </div>
      ...
      {/* Bottom Save Action */}
      <div className="p-6 bg-darker/80 border-t border-white/5 sticky bottom-0 z-30">
        <button 
          onClick={() => { 
            vibrate();
            if (!name.trim()) { alert("Name is required"); return; }
          ...
          className="w-full bg-primary text-white font-black py-5 rounded-2xl uppercase tracking-[0.2em] text-xs shadow-[0_0_40px_rgba(59,130,246,0.3)] active:scale-95 transition-all"
        >
          {mode === 'create' ? 'Build Entity' : 'Finalize Changes'}
        </button>
      </div>
    </div>
  );
...
const CreateView = ({ onCreateCharacter, onCreatePersona, onBack, initialMode }: any) => {
  ...
  if (mode === 'character') {
    return (
      <CharacterEditor 
        mode="create"
        initialData={{ color: 'bg-indigo-600', initial: 'N', creator: '@you', engagement: '0', tags: ['Custom'], maturityLevel: 'teen', visibility: 'public' }}
        onCancel={onBack}
        onSave={(data) => {
          onCreateCharacter({ 
            id: `char_${Date.now()}`, 
            ...
            engagement: '0',
            tags: ['Custom'],
            greeting: `*Manifesting...*\n\n${data.name}: "I exist now. Let's talk."`,
            ...data 
          });
        }}
      />
  ...
  return (
    <div className="flex flex-col h-full bg-darker">
      <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 bg-surface/80 backdrop-blur z-20 sticky top-0">
        <button onClick={onBack} className="p-2 -ml-4 text-slate-400"><ChevronLeft size={24} /></button>
        <h1 className="text-base font-black uppercase tracking-tighter">Architect Persona</h1>
        <button 
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
        <div className="flex bg-surface/50 rounded-2xl p-1 border border-white/5">
        ...
        <div className="space-y-6">
          <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-1">Persona Name</label><input value={personaName} onChange={e => setPersonaName(e.target.value)} className="w-full bg-surface border border-white/5 rounded-2xl p-4 text-white font-bold" /></div>
          <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-1">Traits (comma separated)</label><input value={personaTraits} onChange={e => setPersonaTraits(e.target.value)} className="w-full bg-surface border border-white/5 rounded-2xl p-4 text-white font-bold" /></div>
          <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-1">Bio</label><textarea value={personaBio} onChange={e => setPersonaBio(e.target.value)} className="w-full bg-surface border border-white/5 rounded-2xl p-4 text-white font-medium h-32" /></div>
        </div>
      </div>
    </div>
  );
...
const LibraryView = ({ characters, personas }: any) => (
  <div className="p-4 pb-24">
    <h1 className="text-3xl font-black mb-8 uppercase tracking-tighter">Archive</h1>
    <section className="mb-10">
      <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Saved Personas</h2>
      <div className="grid grid-cols-2 gap-4">{personas.map((p: any) => <div key={p.id} className="bg-surface/30 p-4 rounded-3xl border border-white/5"><div className="font-black text-white text-sm mb-1">{p.name}</div><div className="text-[9px] font-black text-primary uppercase truncate tracking-widest">{p.traits.join(" • ")}</div></div>)}</div>
    </section>
    <section>
      <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Pinned Entities</h2>
      <div className="space-y-3">{characters.filter((c: any) => c.creator === '@you').map((c: any) => <div key={c.id} className="flex items-center gap-4 p-4 bg-surface/30 rounded-3xl border border-white/5"><AbstractAvatar name={c.name} colorClass={c.color} size="sm" initial={c.initial} /><div className="flex-1"><div className="font-black text-sm text-white">{c.name}</div><div className="text-[9px] text-slate-500 font-bold uppercase italic tracking-widest">Self Build</div></div></div>)}</div>
    </section>
  </div>
);

const ProfileView = ({ personas, activePersonaId, setActivePersonaId, chats, updatePersona }: any) => {
  const [editing, setEditing] = useState<any>(null);
  const totalMsgs = chats ? chats.reduce((acc: number, c: any) => acc + c.messages.filter((m: any) => m.role === 'user').length, 0) : 0;
  
  return (
    <div className="p-4 pb-24 relative">
      <div className="flex items-center gap-6 mb-10 pt-4 px-2">
        <div className="w-24 h-24 bg-gradient-to-br from-primary via-indigo-600 to-purple-700 rounded-[2rem] flex items-center justify-center text-4xl font-black shadow-[0_20px_50px_rgba(59,130,246,0.2)] rotate-2 ring-1 ring-white/10">U</div>
        <div><h1 className="text-3xl font-black text-white leading-tight uppercase tracking-tighter">Genesis User</h1><div className="inline-block bg-primary/20 text-primary border border-primary/40 px-2.5 py-0.5 rounded-full font-black text-[9px] uppercase tracking-[0.2em] mt-1 shadow-[0_0_15px_rgba(59,130,246,0.15)]">Lvl. 01 Weaver</div></div>
      </div>
          ...
          {personas.map((p: any) => (
            <div 
              ...
              className={`w-full flex items-center justify-between p-5 rounded-[2.5rem] transition-all cursor-pointer border-2 ${
                ...
                : "bg-surface border-white/5 text-slate-400 hover:border-primary/20"
              }`}
            >
               ...
               <div className="flex items-center gap-3">
                 ...
                 <button onClick={(e) => { e.stopPropagation(); setEditing(p); vibrate(); }} className={`p-2 rounded-xl transition-all ${activePersonaId === p.id ? 'bg-white/10 hover:bg-white/20' : 'bg-white/5 hover:bg-white/10'}`}><Edit3 size={16} /></button>
               </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-8 bg-surface/30 rounded-[3rem] border border-white/5 text-center flex flex-col items-center justify-center"><div className="text-3xl font-black text-white mb-1">{totalMsgs}</div><div className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Echos Sent</div></div>
        <div className="p-8 bg-surface/30 rounded-[3rem] border border-white/5 text-center flex flex-col items-center justify-center"><div className="text-3xl font-black text-white mb-1">∞</div><div className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Memory Nodes</div></div>
      </div>

      {editing && (
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-surface w-full max-w-md rounded-[3rem] border border-white/10 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-8 border-b border-white/5 flex-none"><h2 className="text-xl font-black uppercase tracking-tighter">Morph Identity</h2><button onClick={() => setEditing(null)} className="text-slate-500 p-2"><X size={28} /></button></div>
            <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
              <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Name</label><input value={editing.name} onChange={e => setEditing({...editing, name: e.target.value})} className="w-full bg-darker border border-white/5 rounded-2xl p-4 text-white font-bold" /></div>
              <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Biography</label><textarea value={editing.bio} onChange={e => setEditing({...editing, bio: e.target.value})} className="w-full bg-darker border border-white/5 rounded-2xl p-4 text-white font-medium h-32 leading-relaxed" /></div>
            </div>
            <div className="p-8 border-t border-white/5 bg-surface/50"><button onClick={() => { updatePersona(editing); setEditing(null); vibrate(); }} className="w-full bg-primary text-white font-black py-5 rounded-3xl shadow-xl shadow-primary/20 uppercase tracking-widest text-xs">Commit Manifestation</button></div>
          </div>
        </div>
      )}
    </div>
...
const ChatInterface = ({ session, character, personas, activePersonaId, setActivePersonaId, onBack, onUpdateSession, onUpdateCharacter, onDeleteChat, onCreatePersona }: any) => {
  ...
  return (
    <div className="flex flex-col h-full bg-darker relative">
      <style>{`
        ...
        .glass-header { background: rgba(2, 6, 23, 0.85); backdrop-filter: blur(25px); }
      ...
      {/* Composer */}
      <div className="bg-darker/80 backdrop-blur-3xl border-t border-white/5 px-4 pt-4 pb-10 z-20">
        <div className="flex items-end gap-3 bg-surface/50 p-3 rounded-[2.5rem] border border-white/10 focus-within:border-primary/40 transition-all shadow-inner">
          <textarea 
            value={input} 
      ...
      {showChatMenu && (
        <div className="fixed inset-0 z-30 flex items-start justify-end p-6 pt-24" onClick={() => setShowChatMenu(false)}>
            <div className="w-64 bg-surface border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden backdrop-blur-xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <button onClick={() => { setIsEditing(true); setShowChatMenu(false); vibrate(); }} className="w-full text-left px-6 py-5 hover:bg-white/5 flex items-center gap-4 text-xs font-black uppercase tracking-widest border-b border-white/5"><Settings size={18} /> Character Settings</button>
                <button onClick={() => { setShowMemorySelector(true); setShowChatMenu(false); vibrate(); }} className="w-full text-left px-6 py-5 hover:bg-white/5 flex items-center gap-4 text-xs font-black uppercase tracking-widest border-b border-white/5"><BrainCircuit size={18} /> Etch Memory</button>
      ...
      {isEditing && (
        <CharacterEditor 
          ...
          onSave={(data) => { onUpdateCharacter(data); setIsEditing(false); }} 
          onCancel={() => setIsEditing(false)} 
        />
      )}

      {/* Legacy overlays (Persona, Memory) */}
      {showPersonaMenu && (
        <div className="absolute inset-0 z-[60] bg-black/95 backdrop-blur-2xl flex items-end justify-center animate-in fade-in duration-300">
           ...
           <div className="bg-darker w-full max-h-[85%] rounded-t-[3.5rem] border-t border-white/10 shadow-2xl flex flex-col z-10 animate-in slide-in-from-bottom-20">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                {personas.map((p: any) => (
                  <button key={p.id} onClick={() => { vibrate(); setPendingPersona(p); }} className={`w-full text-left p-6 rounded-[2.5rem] border-2 transition-all flex items-center gap-5 ${activePersonaId === p.id ? "bg-primary border-primary shadow-[0_0_20px_rgba(59,130,246,0.2)]" : "bg-surface border-white/5"}`}>
                    <div className={`w-14 h-14 ${p.avatarColor} rounded-2xl flex-none flex items-center justify-center font-black text-2xl shadow-xl ring-1 ring-white/10`}>{p.name[0]}</div>
      ...
      {pendingPersona && (
        <div className="fixed inset-0 z-[110] bg-black/80 flex items-center justify-center p-8 animate-in fade-in">
          <div className="bg-surface w-full max-w-xs p-10 rounded-[3rem] shadow-2xl border border-white/10 text-center">
            <h4 className="text-xl font-black mb-2 uppercase tracking-tighter">Manifest?</h4>
            <div className="space-y-3">
      ...
      {showMemorySelector && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl flex flex-col animate-in fade-in duration-300">
           <div className="h-20 flex items-center justify-between px-8 border-b border-white/5 flex-none">
              <button onClick={() => setShowMemorySelector(false)} className="text-slate-500 p-2"><X size={28} /></button>
              <h3 className="text-base font-black uppercase tracking-widest">Select Memory Nodes</h3>
              <button onClick={generateMemoryFromSelection} disabled={selectedMsgIds.size === 0 || isSyncingMemory} className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${selectedMsgIds.size > 0 ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white/5 text-slate-700'}`}>{isSyncingMemory ? <RefreshCw className="animate-spin" size={14} /> : 'Crystallize'}</button>
           </div>
           <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
              {session.messages.filter((m: any) => m.role !== 'system').map((msg: any) => (
                <div key={msg.id} onClick={() => { vibrate(); const n = new Set(selectedMsgIds); if (n.has(msg.id)) n.delete(msg.id); else n.add(msg.id); setSelectedMsgIds(n); }} className={`p-6 rounded-[2rem] border transition-all cursor-pointer flex gap-4 ${selectedMsgIds.has(msg.id) ? 'bg-primary/10 border-primary shadow-lg shadow-primary/10' : 'bg-surface/30 border-white/5'}`}>
      ...
      {showMemoryEditor && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-6 animate-in zoom-in-95 duration-200">
           <div className="bg-surface w-full max-w-lg rounded-[3.5rem] border border-white/10 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
              <div className="p-8 border-b border-white/5 flex items-center justify-between"><h3 className="text-xl font-black uppercase tracking-tighter">Memory Node</h3><button onClick={() => setShowMemoryEditor(false)} className="text-slate-500"><X size={24} /></button></div>
              <div className="flex-1 overflow-y-auto p-10 no-scrollbar"><textarea value={generatedMemory} onChange={e => setGeneratedMemory(e.target.value)} className="w-full bg-darker border border-white/5 rounded-3xl p-6 text-white font-medium text-sm leading-relaxed h-[400px] outline-none shadow-inner" /></div>
              <div className="p-10 border-t border-white/5 flex gap-4"><button onClick={() => setShowMemoryEditor(false)} className="flex-1 bg-darker py-5 rounded-[1.5rem] font-black uppercase text-[10px] text-slate-600">Discard</button><button onClick={() => { onUpdateCharacter({ memory: (character.memory ? character.memory + "\n\n" : "") + generatedMemory }); setShowMemoryEditor(false); vibrate(); }} className="flex-2 bg-primary py-5 rounded-[1.5rem] font-black uppercase text-[10px] text-white shadow-xl shadow-primary/20">Commit Node</button></div>
           </div>
        </div>
      )}
    </div>
